...
import { Alert, Avatar, Button, EmailField, HeadSeo, Select, InputField, TextAreaField } from "@quillsocial/ui";
import {
  User as UserIcon,
  Pencil,
  Check,
...
function EditUnchanged({ unchangedValue, updateUnchangedValue }: { unchangedValue: any, updateUnchangedValue: any }) {
    ...
    <div className="flex">
      ...
      {isEditing ? (
        <div className='mr-2 mt-2'>
          <select className={`border rounded w-[163px] border-gray-100 focus:outline-none focus:border-gray-300`}
            onChange={handleDropdownChange} value={selectedValue}>
            <option className='' value="Don't Override">Don't Override</option>
            <option value="Replace Value"
            >Replace Value</option>
          </select>
          {selectedValue === 'Replace Value' && (<div className="mt-2">
            <input
              type="text"
              value={newValue}
