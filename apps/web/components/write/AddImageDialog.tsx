...
export const AddImageDialog: React.FC<AddImageDialogProps> = ({ open, onClose, handleImageChange }) => {
  ...
  const handleSearchImagesUnsplash = async () => {
    ...
    const response = await fetch(`/api/image/getImageUnplash?key=${searchImage}`, {
      credentials: "include",
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      showToast("Error while searching image, please try other keywords", "error");
    }
    setIsButtonSearchImage(false);
  };
    ...
    <Dialog open={open}>
      <DialogContent className="w-full max-w-xl">
        ...
        <div className="flex gap-5 border-b font-bold">
          <button
            className={`flex py-2 ${
              activeTab === SourceType.Upload ? "border-b-2 border-blue-500 text-blue-500" : ""
            }`}
          ...
          <button
            className={`flex py-2 ${
              activeTab === SourceType.Unsplash ? "border-b-2 border-blue-500 text-blue-500" : ""
            }`}
          ...
          <button
            className={`flex py-2 ${
              activeTab === SourceType.Embed ? "border-b-2 border-blue-500 text-blue-500" : ""
            }`}
        ...
        <div className="mt-4">
            ...
            <>
              ...
              <div className="flex flex-col items-center justify-center rounded-lg  border border-gray-300 bg-slate-50 p-4">
                ...
                <ImageUploader
                  ...
                  imageSrc={imageSrc}
                />
                <span className="text-awst m-2 block  text-xs font-bold">Click to Upload</span>
          ...
          {activeTab === SourceType.Unsplash && (
            <div className="mb-2  p-2">
              <p className="text-sm font-semibold text-gray-700">Search Unsplash for free images</p>
                ...
                <div className="">
                  {showUnsplashModal && (
                    <UnsplashImageSelector
                      ...
                      onSelectImage={(imageUrl) => handleImageUnsplashResult(imageUrl)}
          ...
          {activeTab === SourceType.Embed && (
            <div className="mb-2 block p-2">
              <p className="text-sm font-semibold text-gray-700">Paste the image link below</p>
              <div className="relative mt-2 text-gray-600">
                <Input
                  className="border border-gray-300 bg-white pr-16 text-sm focus:outline-none"
                  value={searchLink}
