...
const CalendarPage = () => {
    ...
    const getNameSocial = (inputString: string | undefined) => {
        ...
        const capitalizedFirstWord: string = firstWord.charAt(0).toUpperCase() + firstWord.slice(1);
    ...
    const eventContent = (arg: any) => {
        const { time, description, logo, contentImage, colorStatus } = arg.event.extendedProps;
        return (
            <div style={{ borderLeft: `2px solid ${colorStatus}`, paddingLeft: '1px' }}>
                <div className="flex items-center lg:justify-between justify-start">
                    <img className="h-4 w-4"
                        src={getIconSocial(logo)}
                        alt="Logo"
                    />
    ...
    const handleDatesSet = (dateInfo:any) => {
     ...
     const fetchEventsData = async (view: any, startDate: string, endDate: string) => {
        setIsLoading(true);
        const response = await fetch(`/api/calendar/getAllPost?view=${view}&startDate=${startDate}&endDate=${endDate}`, {
            credentials: "include",
          method: "GET",
          headers: {
        ...
        if (
          ...
          const postsFromDb = data.map((x: any) => {
            const startDay = x.schedulePostDate
              ? dayjs.utc(x.schedulePostDate).tz(timeZone).format("YYYY-MM-DD HH:mm")
              : dayjs.utc(x.postedDate).tz(timeZone).format("YYYY-MM-DD HH:mm");
            const endDay = x.schedulePostDate
              ? dayjs.utc(x.schedulePostDate).add(5, "minute").tz(timeZone).format("YYYY-MM-DD HH:mm")
              : dayjs.utc(x.postedDate).add(5, "minute").tz(timeZone).format("YYYY-MM-DD HH:mm");
                ...
                <div className="bg-white mt-[-50px] px-4 rounded-lg pt-5 pb-3 shadow-lg">
                    ...
                    <div className="mt-2 sm:text-sm text-[8px]">
                        <div className="flex items-center gap-2 ">
                            <div className="flex items-center" ><span className="h-4 w-4 rounded-full bg-blue-500 inline-block mr-2"></span><span>Pending</span></div>
                            <div className="flex items-center"> <span className="h-4 w-4 rounded-full bg-gray-300 inline-block mr-2"></span><span>Draft</span></div>
                            <div className="flex items-center"> <span className="h-4 w-4 rounded-full bg-green-500 inline-block mr-2"></span><span>Published / Sent</span></div>
                            <div className="flex items-center"> <span className="h-4 w-4 rounded-full bg-red-400 inline-block mr-2"></span><span>With errors</span></div>
                            <div className="flex items-center"><span className="h-4 w-4 rounded-full bg-blue-200 inline-block mr-2"></span><span>From autolist</span></div>
                        </div>
                    </div>
                    <div>
                        <Dialog
                                        ...
                                        <div className="grid">
                                            <div className="col-6">
                                                ...
                                                <p className="flex align-items-center items-center mb-2">
                                                    ...
                                                    <span>
                                                        {changedEvent.start?.toString().slice(0, 21)} {`(${timeZone})`}
                                                    </span>
                                                </p>
                                            </div>
                                            <div className="col-6">
                                                ...
                                                <p className="flex align-items-center items-center mb-2">
                                                    <i className="pi pi-fw pi-clock text-700 mr-2"></i>
                                                    <span>{changedEvent.end?.toString().slice(0, 21)} {`(${timeZone})`}</span>
                                                </p>
                                            </div>
                                    ...
                                    <div className="grid p-fluid formgrid">
                                        <div className="col-12 md:col-6 field">
                                            ...
                                            <span className="p-input-icon-left">
                                                ...
                                                <InputText className="pl-[35px] rounded-lg border-slate-200"
                                                    id="title"
                                        ...
                                        <div className="col-12 mt-2 field">
                                            ...
                                            <InputTextarea className="rounded-lg border-slate-200"
                                                id="description"
                                                rows={5}
