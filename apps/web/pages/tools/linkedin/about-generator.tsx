...
const AboutGeneratorView = () => {
  ...
  const mutation = trpc.viewer.generateAbout.useMutation({
    onSuccess: (data) => {
      setAbouts([
        data?.answer1 || "",
        data?.answer2 || "",
      ...
      showToast("Generate about successfully.", "success");
    },
    onError: () => {
...
const AboutGeneratorForm = ({
  ...
  const { t } = useLocale();
  const aboutGeneratorFormSchema = z.object({
    cv: z.string(),
  });

  const formMethods = useForm<FormValues>({
    defaultValues,
    resolver: zodResolver(aboutGeneratorFormSchema),
  });
  ...
  return (
    <>
      ...
      <div className="mx-auto mb-5 grid max-w-2xl auto-rows-fr grid-cols-1 gap-5 sm:mt-20  md:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-2">
...
AboutGeneratorView.PageWrapper = PageWrapper;

export default AboutGeneratorView;
