...
const badgeStyles = cva(
  "font-medium inline-flex items-center justify-center rounded gap-x-1",
  {
    variants: {
      variant: {
...
export const Badge = function Badge(props: BadgeProps) {
  ...
  const isButton =
    "onClick" in passThroughProps && passThroughProps.onClick !== undefined;
  ...
  const Children = () => (
    <>
      {withDot ? <GoDot className="h-3 w-3 stroke-[3px]" /> : null}
  ...
  return React.createElement(
    Wrapper,
    { ...passThroughProps, className: classes },
    <Children />
  );
};
