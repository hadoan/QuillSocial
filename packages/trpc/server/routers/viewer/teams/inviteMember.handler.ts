...
import type { Team, User } from "@quillsocial/prisma/client";
  ...
  try {
    ...
    if (e instanceof Prisma.PrismaClientKnownRequestError) {
      ...
      if (!Array.isArray(input.usernameOrEmail) && e.code === "P2002") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "User was already in the current team",
        });
      } else {
...
export const inviteMemberHandler = async ({ ctx, input }: InviteMemberOptions) => {
  ...
  const getlist: any = await getFirstOrCreateOrgOfUserHandler({ ctx });
  const getbill = await getCurrentUserBillingHanlder({ ctx });
  // if (getbill?.type === BillingType.FREE_TIER && getlist?.members?.length - 1 >= 1) {
  //   throw new TRPCError({
  //     code: "BAD_REQUEST",
  //     message: "Please upgrade your subscription plan to invite new employees.",
  //   });
  // } else if (getbill?.type === BillingType.TEAM && getlist?.members?.length - 1 >= 20) {
  //   throw new TRPCError({
  //     code: "BAD_REQUEST",
  //     message: "Please upgrade your subscription plan to invite new employees.",
  //   });
  // } else if (getbill.type === BillingType.PER_USER && getlist?.members?.length - 1 >= getbill.quantity + 20) {
  //   throw new TRPCError({
  //     code: "BAD_REQUEST",
  //     message: "Please upgrade your subscription plan to invite new employees.",
  //   });
