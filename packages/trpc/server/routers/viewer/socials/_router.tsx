...
export const socialsRouter = router({
  ...
  getSocialNetWorking: authedProcedure.query(async ({ ctx }) => {
    ...
    if (!UNSTABLE_HANDLER_CACHE.getSocialNetWorking) {
      UNSTABLE_HANDLER_CACHE.getSocialNetWorking = await import("./getSocialNetWorking.handler").then(
        (mod) => mod.getSocialHanlder
      );
    }
  ...
  setCurrentUserProfile: authedProcedure.input(ZCurrentUserProfileInputSchema).mutation(async ({ ctx, input }) => {
    if (!UNSTABLE_HANDLER_CACHE.setCurrentUserProfile) {
      UNSTABLE_HANDLER_CACHE.setCurrentUserProfile = await import("./setCurrentUserProfile.handler").then((mod) => mod.setCurrentUserProfileHandler);
    }
  ...
  getSocialConditionsForBilling: authedProcedure.query(async ({ ctx }) => {
    ...
    if (!UNSTABLE_HANDLER_CACHE.getSocialConditionsForBilling) {
      UNSTABLE_HANDLER_CACHE.getSocialConditionsForBilling = await import("./getSocialConditionsForBilling.handler").then(
        (mod) => mod.getSocialConditionsForBillingHanlder
      );
    }
