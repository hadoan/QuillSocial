...
export async function uploadVideo(userId: number, postId: number) {
    ...
    const videoFileName = post.cloudFiles && post.cloudFiles.length > 0 && post.cloudFiles[0].cloudFile?.cloudFileId
        ? `${post.cloudFiles[0].cloudFile?.cloudFileId}.${post.cloudFiles[0].cloudFile?.fileExt}` : undefined;
    if (!videoFileName) {
    ...
    try {
        ...
        if (!res) {
            ...
            await prisma.post.update({
                where: { id: post.id },
                data: {
                    status: "POSTED", postedDate: new Date(), result: { response: JSON.parse(JSON.stringify(res.data)) }
                },
            });
            return true;
        }
