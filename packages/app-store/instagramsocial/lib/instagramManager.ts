import { instagramCredentialSchema } from "./instagramCredentialSchema";
...
export class InstagramProvider {
    ...
    for (let i = 0; i < retryCount; i++) {
      try {
        ...
        if (!response.ok) {
          ...
          const errorResult = this.handleErrors(errorBody);
          if (errorResult?.type === "retry" && i < retryCount - 1) {
  ...
  public handleErrors(body: string):
    | {
  ...
  private checkScopes(requiredScopes: string[], grantedScopes: string[]) {
...
export const post = async (postId: number) => {
  ...
  const credential = await prisma.credential.findUnique({
    where: {
      id: postRecord.credentialId,
    },
  });
  if (!credential || !credential.currentPageId) {
    console.error(
      "Error posting instagram status: credential is null or credential.currentPageId is null"
    );
    return false;
  }
