...
export const generateValuableTips = async (userId: number, topic: string, tips: string, format?: string) => {
  const { chatCompletion: post } = await getRewriteCompletions(userId, getMessageTexts(topic, tips, format));
